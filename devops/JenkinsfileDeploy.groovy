            ...
            try {
                dir(projectDirectory) {
                    slackSend color: "warning",
                            message: "Test pyspark lineage authenticating"
                    jenkinsSteps.argocdAuthenticate(argocdUrl, '${argocdPassword}')
                }
            } catch (err) {
                slackSend color: "danger",
                        message: "Test pyspark lineage failed to authenticate properly"
                throw err
            }
        }
    }
...
stage("Deploy") {
    node('master') {
        try {
            dir(projectDirectory) {
                slackSend color: "warning",
                        message: "Test pyspark lineage deploying"
                ...
                slackSend color: "good",
                        message: "Test pyspark lineage deployed successfully"
            }
        } catch (err) {
            slackSend color: "danger",
                    message: "Test pyspark lineage failed to deploy properly"
            throw err
        }
    }
}
...
stage("Teardown") {
    node('master') {
        try {
            dir(projectDirectory) {
                slackSend color: "warning",
                        message: "Test pyspark lineage shutting down"
                jenkinsSteps.argocdTerminate(argocdAppName)
            }
        } catch (err) {
            slackSend color: "danger",
                    message: "Test pyspark lineage failed to shut down properly"
            throw err
        }
    }
}
